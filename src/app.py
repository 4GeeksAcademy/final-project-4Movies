...
def login():
    ...
    is_blacklisted_token = TokenBlocklist.check_blacklist(access_token)
    if is_blacklisted_token:
        return jsonify({"msg": "Token blacklisted. Please log in again."}), 401
...
def logout():
    ...
    if "authToken" not in body:
        return jsonify("Missing parameter 'authToken'"), 400

    # Check if the access token is blacklisted
    is_blacklisted_token = TokenBlocklist.check_blacklist(body["authToken"])
    if is_blacklisted_token:
        return jsonify({"msg": "Token blacklisted. Please log in again."}), 401
...
def update_password():
    try:
        ...
        if data is None or "password" not in data:
            return (
                jsonify({"error": "Invalid JSON data. 'password' field is required."}),
                400,
            )
        ...
        # Validate the new password (adjust the criteria accordingly)
        ...
        # Check if the access token is blacklisted
        is_blacklisted_token = TokenBlocklist.check_blacklist(jti)
        if is_blacklisted_token:
            return jsonify({"msg": "Token blacklisted. Please request a new one."}), 401
...
def get_user():
    ...
    return (jsonify({"msg": "The obtained user is:", "results": serialized_user}), 200)
...
def get_movies():
    ...
    try:
        ...
        # Directamente devolver la respuesta de la API podría ser suficiente
        # ya que incluye page, total_pages, y total_results
...
@app.route("/getproviders/<int:id>", methods=["GET"])
def get_providers(id):
    ...
    # Specify the TMDb API endpoint with the specific movie ID
    tmdb_api_url = f"{base_url}/movie/{id}/watch/providers?api_key=" + api_key
    ...
    try:
        ...
        # Return the provider details as JSON response
        return (
            jsonify(
                {"msg": "Providers obtained successfully", "results": provider_data}
            ),
            200,
        )
        ...
        return (
            jsonify({"msg": f"Error fetching providers from TMDb API: {str(e)}"}),
            500,
        )


@app.route("/gettrailer/<int:id>", methods=["GET"])
def get_trailer(id):
    # Retrieve TMDb API key and base URL from environment variables
    api_key = os.getenv("API_KEY")
    base_url = os.getenv("APIMOVIES_URL")
    ...
    # Specify the TMDb API endpoint with the specific movie ID
    tmdb_api_url = f"{base_url}/movie/{id}/videos?api_key=" + api_key
    ...
    try:
        ...
        # Return the provider details as JSON response
        return (
            jsonify({"msg": "Trailer obtained successfully", "result": trailer_data}),
            200,
        )
...
def get_reviews():
    try:
        # Fetch all reviews
        reviews = Movie_Review.query.all()
...
@app.route("/favoritemovies/", methods=["GET"])
@jwt_required()
def get_personal_movies():
    current_user_id = get_jwt_identity()
    # Retrieve movies from the user's personal list with a specific view status
    personal_movies = Personal_List.query.filter_by(user_id=current_user_id).all()

    # Extract movie information (you may adjust this based on your model structure)
    movies_list = [entry.movie_id for entry in personal_movies]
...
def change_view_status(id):
    ...
    entry_to_update.view_state = "Visto"
...
def get_actors():
    try:
        ...
        # Function to fetch actors from a single page
        def fetch_actors(page):
        ...
        total_pages = 1  # Start with one page
        actors_list = []

        # Fetch actors from multiple pages using ThreadPoolExecutor
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit tasks to the executor for each page
            futures = [
                executor.submit(fetch_actors, page)
                for page in range(1, total_pages + 1)
            ]

            # Gather the results from completed tasks
            for future in concurrent.futures.as_completed(futures):
                try:
                    actors_list.extend(future.result())
                except requests.exceptions.RequestException as e:
                    # Handle exceptions for individual pages
                    return (
                        jsonify(
                            {"msg": f"Error fetching actors from TMDb API: {str(e)}"}
                        ),
                        500,
                    )

        # Return the list of all actors as JSON response
        return (
            jsonify(
                {"msg": "All actors obtained successfully", "results": actors_list}
            ),
            200,
        )
...
def get_actor_id(id):
    ...
    # Specify the TMDb API endpoint with the specific movie ID
    tmdb_api_url = f"{base_url}/person/{id}?api_key=" + api_key
